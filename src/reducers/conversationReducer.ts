...
const reducer = (state: any = INITIAL_STATE, action: any) => {
  switch (action.type) {
    ...
    case CONVERSATION_ACTIONS.RECEIVE_NEW_MESSAGE: {
      const olderConversations = state.conversations.filter(
        (conv: any) => conv._id !== action.payload.id,
      );
      const newestConv = state.conversations.filter(
        (conv: any) => conv._id === action.payload.id,
      )[0];
      const {
        email,
      ...
      return {
        ...
        total: {
          ...state.total,
          [action.payload.id]: state.total[action.payload.id] + 1,
        },
        lastMessages,
    ...
    case CONVERSATION_ACTIONS.SEEN_LAST_MESSAGE: {
      return {
        ...
        lastMessages: {
          ...state.lastMessages,
          [action.payload.id]: {
            ...state.lastMessages[action.payload.id],
            seen: false,
            totalUnseen: 0,
          },
        },
      ...
      return {
        ...
        nrMessages: {
          ...
          [action.payload.conversationId]:
            ...
            state.nrMessages[action.payload.conversationId].length > 0
              ? [
                  ...state.nrMessages[action.payload.conversationId],
                  {
        ...
        nrMessagesLoadings: {
          ...state.nrMessagesLoadings,
          [action.payload.conversationId]:
            state.nrMessagesLoadings[action.payload.conversationId] &&
            state.nrMessagesLoadings[action.payload.conversationId].length > 0
              ? [
                  ...state.nrMessagesLoadings[action.payload.conversationId],
                  {
                    active: true,
    ...
    case CONVERSATION_ACTIONS.DELETE_NR_MESSAGE: {
        ...
        if (conv === action.payload.conversationId && state.nrMessages[conv]) {
          state.nrMessages[conv].forEach((m: any) => {
            if (m.messageId !== action.payload.id) {
              objM[conv] =
                objM[conv] && objM[conv].length > 0 ? [...objM[conv], m] : [m];
            }
          });
        } else {
      ...
      Object.keys(state.nrMessagesLoadings).forEach((conv: any) => {
        if (
          conv === action.payload.conversationId &&
          state.nrMessagesLoadings[conv]
        ) {
    ...
    case CONVERSATION_ACTIONS.OTHER_SEEN_LAST_MESSAGE: {
      return {
        ...
        lastMessages: {
          ...state.lastMessages,
          [action.payload.conversationId]: {
            ...state.lastMessages[action.payload.conversationId],
            seenByOther: true,
          },
        },
      };
