...
function modalData(element) {
    ...
    fetch(`https://api.themoviedb.org/3/movie/${element.id}?api_key=${API_KEY}&language=en-US`).then(res => res.json()).then(production => {
        ...
        imdb.setAttribute("href", `https://www.imdb.com/title/${production.imdb_id}/?ref_=nv_sr_srsg_1`)
...
function addFavoriteList() {
    ...
    favoriteMoviesArray.forEach(x => {
        ...
        favMovieImg.setAttribute("src", `https://image.tmdb.org/t/p/w500/${x.poster_path}`)
...
function searchMovie(event) {
    ...
    fetch(`https://api.themoviedb.org/3/search/movie?query=${value}&api_key=${API_KEY}`).then(res => res.json()).then(data => {
        console.log(data);
