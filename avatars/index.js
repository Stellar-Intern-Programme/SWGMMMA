...
window.addEventListener('load', () => {
  ...
  avatarPictures?.addEventListener(
    'scroll',
...
function getAvatars(offset = 0, search = false) {
  ...
  fetch(
    'https://gateway.marvel.com:443/v1/public/characters?apikey=' +
      ...
      '&limit=100&offset=' +
      offset +
      (searchInputValue?.length ? '&nameStartsWith=' + searchInputValue : ''),
  )
    .then(response => {
...
function createImages(result) {
  ...
  img.setAttribute(
    'src',
    'https' +
      result.thumbnail.path.split('http').join('') +
      '.' +
...
function avatarSelect(e) {
  ...
  document.querySelector('.pop-up p').textContent =
    e.target.getAttribute('avatar-name');
}
...
function changeAvatar() {
  ...
  parent?.postMessage?.(JSON.stringify([imgSrc, 'add']));
}
