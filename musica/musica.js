...
function musicFetch() {
  fetch(
    'https://ws.audioscrobbler.com/2.0/?method=chart.gettoptracks&api_key=ddc75f55ff59c84a1b4f11fefc6313a9&format=json',
  )
    .then(res => res.json())
    .then(data => {
...
function albumFetch(artist, title, img, def) {
  fetch(
    `https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=ddc75f55ff59c84a1b4f11fefc6313a9&artist=${artist}&track=${title}&format=json`,
  )
    .then(res => res.json())
    .then(data => {
...
function generateTrending(data) {
  const trendShi = data.tracks.track.map(track => ({
    ...track,
...
function musicSearhFetch() {
  ...
  fetch(
    'http://ws.audioscrobbler.com/2.0/?method=track.search&track=' +
      value +
      '&api_key=ddc75f55ff59c84a1b4f11fefc6313a9&format=json',
  )
    .then(res => res.json())
    .then(data => {
